...
//        int[][] stockPrices = {{1, 1},{499999999, 2},{999999998, 3},{1000000000, 5}};
    ...
    public static int minimumLines(int[][] stockPrices) {
        ...
        int diff_a = stockList.get(1).price - stockList.get(0).price;
        int diff_b = stockList.get(1).day - stockList.get(0).day;

        for (int i = 2; i < stockList.size(); i++) {
            Stock prev = stockList.get(i - 1);
            Stock now = stockList.get(i);

            int tmp_a = now.price - prev.price;
            int tmp_b = now.day - prev.day;

            if((diff_a == tmp_a && diff_b == tmp_b) || (diff_a * tmp_b == diff_b * tmp_a)) {
                continue;
            }
            lines++;
